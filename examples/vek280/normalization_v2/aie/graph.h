...
public:
	SimpleGraph() {
		in=input_plio::create("PLIO_01_TO_AIE", plio_128_bits,  "data/input0.csv");
		out=output_plio::create("PLIO_02_FROM_AIE", plio_128_bits,  "data/output0.txt");
    	...
    	connect(mtxA.out[1], k_deviation.in[0]);
    	connect(mtxA.out[2], k_norm.in[0]);
		...
		dimensions(k_mean.out[0])={2};
		dimensions(k_deviation.in[0])={K_ROW*K_COL};
		dimensions(k_deviation.in[1])={2};
		dimensions(k_deviation.out[0])={2};
		dimensions(k_norm.in[0])={K_ROW*K_COL};
		dimensions(k_norm.in[1])={2};
		dimensions(k_norm.out[0])={K_ROW*K_COL};
